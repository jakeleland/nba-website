...
$.ajax(standings).done(function (response) {
	...
	for(let i = 0; i < 15; i++) {
		let team = response['api']['standings'][i]['teamId'];
		let wins = response['api']['standings'][i]['win'];
		let loss = response['api']['standings'][i]['loss'];
		let gamesBehind = response['api']['standings'][i]['gamesBehind'];
		let winPercentage = response['api']['standings'][i]['winPercentage'];
		let conferanceRank = response['api']['standings'][i]['conference']['rank'];
		console.log(team, wins, loss, gamesBehind, winPercentage, "i = " + i, conferanceRank);
		if (team == 21) {
			team = 'Raptors'
		}
		else if (team == 38){
		...
		document.getElementById('team'+conferanceRank).innerHTML = team;
		document.getElementById('team'+conferanceRank+'-wins').innerHTML = wins;
		document.getElementById('team'+conferanceRank+'-loses').innerHTML = loss;
		document.getElementById('team'+conferanceRank+'-win-percentage').innerHTML = winPercentage;
		document.getElementById('team'+conferanceRank+'-games-behind').innerHTML = gamesBehind;
	}
});